...
const SearchPage: React.FC=() =>{
    ...
    const isYouTubeUrl = (q: string) => /(?:youtu\.be\/|youtube\.com\/.*v=)([^&]+)/.test(q);
                ...
                {query.trim() !== '' && (
                    isYouTubeUrl(query) ? (
                        <div className="mt-4 flex space-x-2">
                            <button
                                onClick={() => {
                                    const match = query.match(/(?:youtu\.be\/|youtube\.com\/.*v=)([^&]+)/);
                        ...
                        <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-gray-800 text-black dark:text-white rounded shadow-lg z-50 max-h-80 overflow-auto">
                            {results.length === 0 ? (
                                <p className="p-4 text-sm text-gray-700 dark:text-gray-300">{query.trim() !== "" ? "Aucun résultat" : ""}</p>
                            ) : (
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
                                    {results.map((video, index) => (
                                        <button
                                            ...
                                            className="group flex items-center space-x-2 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
                                        >
                                            <img src={video.thumbnail} alt={video.title} className="w-16 h-10 object-cover rounded" />
                                            <span className="text-sm text-gray-900 dark:text-gray-100">{video.title}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
